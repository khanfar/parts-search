...
def generate_report(start_date, end_date):
    ...
    # Iterate over the final records and extract the values after each '#'
    for _, row in final_records.iterrows():
        final_report = row['تقرير نهائي']
        if '#' in final_report:
            # Split the text at the '#' sign and take the parts after it
            parts_after_hash = final_report.split('#')
            values_in_report = []
            for part in parts_after_hash[1:]:  # Skip the part before the first #
            ...
            # Sum all extracted values for this report
